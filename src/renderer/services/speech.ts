...
export class SpeechService {
  ...
  async start(config: SpeechConfig, onResult: OnResult, onError: OnError): Promise<void> {
    ...
    logger.log('[Speech] Starting with engine:', config.engine, 'language:', config.language);
  ...
  private startBrowserSpeech(config: SpeechConfig): void {
    ...
    if (!SpeechRecognitionClass) {
      logger.warn('[Speech] Web Speech API not available');
    ...
    recognition.onerror = (event: SpeechRecognitionErrorEvent): void => {
      ...
      logger.warn('[Speech] Browser speech error:', event.error);
      ...
      if (FALLBACK_ERRORS.includes(event.error) && !this.triedWhisperFallback) {
        ...
        logger.log('[Speech] Auto-falling back to Whisper...');
        ...
        this.startWhisperCapture({ ...fallbackConfig, engine: 'whisper' }).catch((err) => {
          logger.error('[Speech] Whisper fallback failed:', err);
    ...
    recognition.onend = (): void => {
      ...
      if (this.isActive && this.restartCount < this.maxRestarts) {
        ...
        logger.log(`[Speech] Recognition ended, restarting in ${delay}ms (attempt ${this.restartCount})`);
        setTimeout(() => {
          if (this.isActive && !this.triedWhisperFallback) {
            try {
              recognition.start();
            } catch (err) {
              logger.error('[Speech] Failed to restart browser recognition:', err);
            }
          }
        }, delay);
      }
    ...
    try {
      recognition.start();
      logger.log('[Speech] Browser speech recognition started');
    } catch (err) {
      logger.error('[Speech] Failed to start browser recognition:', err);
  ...
  private async startWhisperCapture(config: SpeechConfig): Promise<void> {
    try {
      logger.log('[Speech] Starting Whisper capture...');
      ...
      recorder.onstop = async (): Promise<void> => {
        ...
        if (this.audioChunks.length === 0) {
          logger.warn('[Speech] Whisper: no audio chunks captured');
          return;
        }
      ...
      recorder.onerror = (e: Event): void => {
        logger.error('[Speech] Whisper: MediaRecorder error:', e);
      };

      this.mediaRecorder = recorder;
      // Emit chunks every 5 seconds for interim processing
      recorder.start(5000);
      logger.log('[Speech] Whisper MediaRecorder started, state:', recorder.state);
    } catch (err) {
      logger.error('[Speech] Failed to start Whisper capture:', err);
  ...
  private async transcribeWithWhisper(audioBlob: Blob, language: string): Promise<void> {
    try {
      ...
      if (!key) {
        logger.error('[Speech] Whisper: no OpenAI API key found');
      ...
      if (!response.ok) {
        ...
        logger.error('[Speech] Whisper API error:', response.status, response.statusText);
      ...
      logger.error('[Speech] Whisper transcription failed:', err);
