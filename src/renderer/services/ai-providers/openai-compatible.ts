...
/**
 * Static config for an OpenAI-compatible chat provider.
 * OpenAI, Groq, and OpenRouter all speak the same /chat/completions wire format,
 * so they share this one adapter — only the base URL, headers, and model list differ.
 */
export interface OpenAICompatibleConfig {
  id: ProviderID;
  name: string;
  models: ModelConfig[];
  /** Omit for OpenAI's default endpoint; set for Groq / OpenRouter. */
  baseURL?: string;
  /** e.g. OpenRouter's HTTP-Referer / X-Title attribution headers. */
  defaultHeaders?: Record<string, string>;
  /** Cheap model used by validateKey(); defaults to models[0].id. */
  validationModel?: string;
}

/**
 * Base class implementing the full AIProvider contract against any
 * OpenAI-compatible endpoint. Subclasses only supply an OpenAICompatibleConfig.
 */
export class OpenAICompatibleProvider implements AIProvider {
  ...
  protected readonly validationModel: string;
  ...
  constructor(config: OpenAICompatibleConfig) {
    ...
    this.validationModel = config.validationModel ?? config.models[0]?.id;
  }
  ...
  async validateKey(): Promise<ValidationResult> {
    ...
    try {
      ...
      console.error(`[${this.name}] validateKey error:`, { status: err.status, message: err.message, name: err.name });
  ...
  async *chat(request: ChatRequest): AsyncGenerator<StreamChunk, ChatResponse> {
    if (!this.client) throw new Error(`${this.name} not initialized`);
      ...
      if (msg.role === 'user' && msg.images && msg.images.length > 0) {
        ...
        for (const img of msg.images) {
          if (!img.data || typeof img.data !== 'string') {
            console.error(`[${this.name}] Invalid image data in message:`, {
      ...
      if (lastUserMsg && lastUserMsg.role === 'user') {
        ...
        if (!alreadyHasImages) {
          ...
          for (const img of request.images) {
            if (!img.data || typeof img.data !== 'string') {
              console.error(`[${this.name}] Invalid request-level image data:`, {
                mimeType: img.mimeType,
                dataType: typeof img.data,
                dataIsNull: img.data === null,
              });
